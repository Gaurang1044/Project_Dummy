...
def detectFace(img):
    ...
    if len(faces) > 0:
        ...
        img = img[int(face_y):int(face_y+face_h),
                  int(face_x):int(face_x+face_w)]
...
def alignFace(img):
    ...
    if len(eyes) >= 2:
        ...
        raise Exception("Unclear Eyes in Photo")
    return new_img
...
if os.path.isfile(face_detector_path) != True:
    raise ValueError(
        "Confirm that opencv is installed on your environment! Expected path  violated.")
...
test_set = glob(os.path.join(dir, '*.jpg'))  # glob(testing_samples_dir)
print(len(test_set))
i = 0
x=0
for instance in test_set:
    ...
    try:
        ...
        #     gender = 0
        # age = ImgName[6:8]
        # NName = str(age) + '_' + str(gender) + '_' + str(i)+".jpg"
    ...
    # status1 = cv2.imwrite('/save/1.jpg', img)
    # status = cv2.imwrite('/save/g_'+instance, gray_img)
    # print("Image written to file-system : ", status1)
    # print("Gray Image written to file-system : ", status)
